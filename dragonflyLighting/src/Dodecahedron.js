class Dodecahedron {
    ...
    render(){ 
        ...
        for (var i = 0; i < faces.length; i++) {
            ...
            // Use face centroid direction as outward normal (reliable for a convex
            // shape centered at origin regardless of winding order)
            var cx = (v0[0]+v1[0]+v2[0]+v3[0]+v4[0]) / 5;
            var cy = (v0[1]+v1[1]+v2[1]+v3[1]+v4[1]) / 5;
            var cz = (v0[2]+v1[2]+v2[2]+v3[2]+v4[2]) / 5;
            var nlen = Math.sqrt(cx*cx + cy*cy + cz*cz) || 1;
            var nx = cx/nlen, ny = cy/nlen, nz = cz/nlen;
            var n = [nx,ny,nz, nx,ny,nz, nx,ny,nz];
            var uv = [0,0, 0,0, 0,0];
